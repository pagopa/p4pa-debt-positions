...
class DebtPositionDeletionServiceImplTest {
  ...
  void givenDebtPositionWithInstallmentNotifiedWhenDeleteThenThrowException() {
    ...
    assertEquals("[DP_ALREADY_NOTIFIED] DebtPosition with id 1 cannot be deleted because it is been notified", exception.getMessage());
  ...
  void givenDebtPositionInPaidStatusWhenDeleteThenThrowException() {
    ...
    assertEquals("[DP_STATUS_NOT_DELETABLE] DebtPosition with id 1 cannot be deleted because is not in allowed status: PAID", exception.getMessage());
