...
class IuvSequenceNumberServiceImplTest {
  ...
  void givenValidOrgWhenGetNextIuvSequenceNumberThenOk(){
    ...
    Mockito.when(iuvSequenceNumberRepository.findIuvSequenceNumberByOrganizationId(1L)).thenReturn(Optional.of(iuvSequenceNumber));
    ...
    Mockito.verify(iuvSequenceNumberRepository, Mockito.times(1))
      .findIuvSequenceNumberByOrganizationId(1L);
  }

  @Test
  void givenNonExistentOrgWhenGetNextIuvSequenceNumberThenOk(){
    Mockito.when(iuvSequenceNumberRepository.findIuvSequenceNumberByOrganizationId(1L)).thenReturn(Optional.empty());
