...
public class InstallmentSynchronizePaymentOptionApplierService {
  ...
  public void merge(InstallmentSynchronizeDTO installmentSynchronizeDTO, PaymentOptionDTO paymentOptionDTO){
    ...
    if (!modifiedFields.isEmpty()) {
      throw new ConflictErrorException(String.format("[UNMODIFIABLE_FIELD] These fields for payment option with index %s of debt position with iupd %s are not mutable: %s", paymentOptionDTO.getPaymentOptionIndex(), installmentSynchronizeDTO.getIupdOrg(), modifiedFields));
    }
  }
}
