...
public class DebtPositionManageApplierService {
  ...
  public void merge(InstallmentDTO updatedInstallment, InstallmentDTO storedInstallment) {
    ...
    if (storedInstallment.getTransfers().size() != updatedInstallment.getTransfers().size()) {
      throw new ConflictException(ErrorCodeConstants.ERROR_CODE_IMMUTABLE_FIELD, String.format("The number of beneficiary for installment having id %s cannot be modified", storedInstallment.getInstallmentId()));
    }
    ...
    storedInstallment.getTransfers()
      .forEach(transferDTO -> {
        if (mapIndexTransferUpdated.get(transferDTO.getTransferId()) == null) {
          throw new ConflictException(ErrorCodeConstants.ERROR_CODE_TRANSFER_NOT_FOUND, String.format("The transfer having id %s of installment having id %s does not found", transferDTO.getTransferId(), storedInstallment.getInstallmentId()));
        }
