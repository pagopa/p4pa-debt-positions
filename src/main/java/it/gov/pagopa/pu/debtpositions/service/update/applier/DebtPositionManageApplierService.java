...
public class DebtPositionManageApplierService {
  ...
  public void merge(InstallmentDTO updatedInstallment, InstallmentDTO storedInstallment) {
    ...
    if (!modifiedFields.isEmpty()) {
      throw new ConflictErrorException("IMMUTABLE_FIELD", String.format("These fields for installment having id %s are not mutable: %s", storedInstallment.getInstallmentId(), modifiedFields));
    }

    if (storedInstallment.getTransfers().size() != updatedInstallment.getTransfers().size()) {
      throw new ConflictErrorException("IMMUTABLE_FIELD", String.format("The number of beneficiary for installment having id %s cannot be modified", storedInstallment.getInstallmentId()));
    }
    ...
    storedInstallment.getTransfers()
      .forEach(transferDTO -> {
        if (mapIndexTransferUpdated.get(transferDTO.getTransferId()) == null){
          throw new ConflictErrorException("TRANSFER_NOT_FOUND", String.format("The transfer having id %s of installment having id %s does not found", transferDTO.getTransferId(), storedInstallment.getInstallmentId()));
        }
  ...
  private void mergeTransfer(TransferDTO storedTransfer, TransferDTO updatedTransfer, Long installmentId) {
    ...
    if (!modifiedFields.isEmpty()) {
      throw new ConflictErrorException("IMMUTABLE_FIELD", String.format("These fields for transfer with index %s of installment having id %s are not mutable: %s",
